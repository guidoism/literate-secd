...
@d mksymbol(h, t) (cell){((((uint64_t)h | 0x80000000) << 33) | (((uint64_t)t & 0x7fffffff) << 2) | 0b11)}
...
void validate_ldc_implementation(void) @+ {
    ...
    mem[103] = mknumber(42);
    index w = popc();
    assert(getnumber(w) == 2); // LDC
    ldc();
    assert(getnumber(head(mem[s])) == 42);
}
...
void validate_atom_implementation(void) @+ {
    ...
    f = 9;
    index w = popc();
    printf("GUIDO: index w = %u\n", w);
    printf("GUIDO: mem[w] = %llu\n", mem[w].val);
    printf("GUIDO: mem[w] >> 2 = %llu\n", mem[w].val >> 2);
    assert(getnumber(w) == 12); // ATOM
    atom();
    assert(head(mem[s]) == F);
    atom();
    assert(head(mem[s]) == T);
...
void validate_add_implementation() @+ {
    ...
    f = 10;
    index w = popc();
    assert(getnumber(w) == 15); // ADD
    add();
    assert(getnumber(head(mem[s])) == 12);
}
...
void validate_sub_implementation() @+ {
    ...
    f = 10;
    index w = popc();
    assert(getnumber(w) == 16); // SUB
    sub();
    assert(getnumber(head(mem[s])) == 2);
}
...
void validate_mul_implementation() @+ {
    ...
    f = 10;
    index w = popc();
    assert(getnumber(w) == 17); // MUL
    mul();
    assert(getnumber(head(mem[s])) == 35);
}
...
void validate_div_implementation() @+ {
    ...
    f = 10;
    index w = popc();
    assert(getnumber(w) == 18); // DIV
    div();
    assert(getnumber(head(mem[s])) == 1);
}
...
int64_t getnumber(index i) @+ {
    return (int64_t)(mem[i].val) >> 2;
}
...
index locate(index pair) @+ {
    ...
    int64_t frame    = getnumber(head(mem[pair]));
    int64_t position = getnumber(tail(mem[pair])); // Is ordinal a better name?
...
void eq(void) @+ {
    index x = pops();
    index y = pops();
    bool b = x == y || getnumber(x) == getnumber(y);
    s = addcons(b ? T : F, s);
}
#+end_src
...
#+begin_src c
void add(void) @+ {
    int64_t x = getnumber(pops());
    int64_t y = getnumber(pops());
...
void leq(void) @+ {
    index x = pops();
    index y = pops();
    bool b = getnumber(y) <= getnumber(x);
    s = addcons(b ? T : F, s);
}
#+end_src
